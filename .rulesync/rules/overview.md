...
- **Error visibility**: Favor loud, obvious error reporting over quiet logs, swallowed exceptions, or silent fallback behavior. In editor/UI code, caught or recoverable errors should usually be logged through user-visible notifications so the failure is immediately diagnosable.
- **Request error handling**: Any editor-facing request path (external CLI, Git/GitHub API, network/API call, async metadata lookup, completion source, or background process) must have explicit failure handling that reports request errors through notifications (`vim.notify` or the module's notification wrapper). Include the actionable stderr/API/decode message. Never silently collapse request failures into empty results, loading states, or no-ops; tests should cover the failure notification path.
- **Commit subject formatting**: Any status, picker, preview, header, or detail row that displays text starting with a commit subject must use the shared conventional commit formatter/highlighter instead of hand-building a plain string. In DiffReview, route these through `M._status_conventional_commit_subject_segments(...)` / `M._status_conventional_commit_type_end(...)` so conventional commit `<type>` text such as `feat:` is colorized consistently across Head/Merge/Push, PR, About, and commit-list rows.
- **Mixed Neovim buffers and markdown**: Do not register an entire synthetic mixed-content filetype, such as `GitStatus`, as `markdown` just to render one markdown subsection. Markdown Tree-sitter syntax can then overlay unrelated diff/code rows with groups such as `Special` or `@markup.*` and hide the intended language highlighting. Scope markdown/otter/render-markdown activation to the actual markdown region or explicit markdown buffer, and verify rendered cells with `nvim__inspect_cell` when debugging highlight conflicts.
- **DiffReview hunk rendering**: Keep raw git hunks and displayed hunks separate. Raw hunks should come from zero-context git diffs and remain the source of truth for staging, unstaging, viewed state, jumping, comments, and future line actions. Presentation code may inject Tree-sitter-aware context and merge adjacent/overlapping display windows, but that virtual grouping must preserve `raw_hunks` metadata and must not become the backing patch/action unit by accident.
