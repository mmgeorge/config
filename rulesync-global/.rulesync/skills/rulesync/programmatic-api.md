...
| `inputRoot`         | `string`       | `process.cwd()`   | Directory containing the `.rulesync/` source files. Output still goes to each `outputRoots` entry, and only the input source root is redirected. Mirrors the CLI's `--input-root`. |
