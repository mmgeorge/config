...
- **Top-down declaration order.** Organize every module in `caller -> callee` flow so reading from top to bottom moves from its public purpose into progressively lower-level implementation detail. Within each declaration tier, place a caller, owner, or parent component before the classes, functions, hooks, or child components it depends on. Put each dependency directly beneath its first meaningful consumer when practical.
  - **TypeScript and JavaScript:** After imports, use this strict priority: type aliases, interfaces, and enums → all module-level `const` declarations, public or private → public classes → public functions → private classes → private functions. Within the `const` tier, keep parent components before child components and keep related configuration constants near the component or API they configure.
  - **Rust:** After imports and module declarations, place type aliases, traits, and enums first, followed by constants and statics. Then organize concrete owners in `struct` → associated `impl` blocks → free functions order. Put public structs and their impls before public free functions, then private structs and their impls before private free functions. Keep each impl immediately beneath the struct it implements when practical.
  - **Dependency flow:** Preserve `caller -> callee` order at every level. Never place a helper above its primary caller merely because the helper is shorter. A reader should encounter the main workflow first, then its direct dependencies, then deeper dependencies.
