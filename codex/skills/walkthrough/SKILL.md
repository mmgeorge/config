...
Do not delegate this task to a custom agent. Own the full workflow from context gathering through validation and repair.
...
6. Validate the generated JSON:
   - Parse it as JSON.
   - Validate it against `walkthrough.schema.json` from this skill directory, or the target repository's DiffReview schema when that schema is present and more local to the consumer.
