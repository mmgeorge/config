...
local function remote_editor_cmd()
  ...
  local lua_cmd = vim.fn.shellescape("lua require('diff_review.integrations.commit').client()")
  return table.concat({
...
function M.client()
  local ok, err = pcall(function()
    ...
    local code = ("lua require('diff_review.integrations.commit').editor(%q, %q)"):format(target, client_addr)
...
function M.editor(target, client_addr)
  ...
  require("diff_review.integrations.ai_commit").populate_commit_buffer_when_ready(buf, st.root or vim.fn.getcwd(), function(message, level)
