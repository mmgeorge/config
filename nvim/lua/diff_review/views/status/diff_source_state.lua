...
--- builders, pr_overview, and live status state from session.lua and sibling modules.
...
local hunk_index_mod = require("diff_review.render.hunk_index")
...
function M._status_configure_diff_source_policies(registry)
  ...
  source.set_kind_policy(registry, "staged", {
    unstage = true,
    discard = true,
    jump = true,
  })
  source.set_kind_policy(registry, "pr", {
    comment = true,
    jump = true,
  })
  source.set_kind_policy(registry, "review", {
    comment = true,
    viewed = true,
    unviewed = true,
    jump = true,
  })
  source.set_kind_policy(registry, "commit", {
    jump = true,
  })
  source.set_kind_policy(registry, "branch", {
    jump = true,
  })
  source.set_kind_policy(registry, "walkthrough", {
    jump = true,
  })
end
...
function M._status_commit_source_handle(commit, status)
  ...
  return {
    ...
    load = function(source_state, done)
      trace.event("source.commit.load.start", status.buf, {
        source_id = source_id,
        commit_oid = commit.oid,
      })
      git_backend.systemlist_async(git_backend.git_show_diff_command(cwd, commit.oid), function(output, code, stderr)
        if code ~= 0 then
        ...
        local files = trace.span("source.commit.parse", status.buf, {
          source_id = source_id,
          diff_len = #diff_text,
        }, function()
          return section_map._status_commit_files_from_diff(cwd, commit, diff_text)
        end)
        ...
        trace.event("source.commit.load.done", status.buf, {
          source_id = source_id,
          file_count = #files,
        })
        done(true)
...
function M._status_register_commit_source_handle(commit)
  ...
  source.ensure_handle(status.diff_source_registry, handle)
...
function M._status_ensure_commit_source_state(commit)
  ...
  local loader = source_loader.ensure(status.diff_source_registry, handle)
  commit.diff_source_id = handle.id
...
function M._status_add_diff_file_annotation(file_state, comment, kind)
  ...
  local annotation = annotations.upsert(file_state.annotation_index, {
    id = id,
    kind = kind,
...
function M._status_set_diff_file_text_loaders(file_state, file, status, source_kind)
  ...
  if source_kind == "commit" and file.diff_source_id then
    ...
    source.set_text_loader(file_state, "new", function(done)
    ...
    source.set_text_loader(file_state, "old", function(done)
...
function M._status_ensure_diff_file_state(file, entry_kind, hunk_entry_kind, file_key)
  ...
  local file_state = source_loader.ensure_file(registry, source_handle, file_path, {
    ...
    expanded = fold_state._status_folded(file_key, false) == false,
    metadata = {
      status_file = file,
      file_key = file_key,
  ...
  file_state.expanded = fold_state._status_folded(file_key, false) == false
  ...
  file_state.syntax_context = syntax_context.ensure_context(file_state.syntax_context, file_state.key)
...
function M._status_populate_diff_file_hunks(file_state, file, hunks)
  ...
  for hunk_index, hunk in ipairs(hunks or {}) do
    ...
    hunk.raw_hunks = hunk.raw_hunks or source.raw_hunks_from_diff(hunk.diff or "", {
    ...
    for _, raw_hunk in ipairs(hunk.raw_hunks or {}) do
      source.add_raw_hunk(file_state, raw_hunk)
    end
  end
end
...
function M._status_populate_commit_source_files(source_state, commit, files, status)
  ...
  for _, file in ipairs(files or {}) do
    ...
    local file_key = status_keys.commit_file_key(commit.oid, file.filename)
    local file_path = M._status_diff_file_path(file, status)
    local file_state = source.ensure_file(source_state, file_path, {
      original_path = file.original_relpath,
      status = file.git_status or file.status,
      stage_state = "readonly",
      added = file.added,
      removed = file.removed,
      expanded = fold_state._status_folded(file_key, true) == false,
      metadata = {
        status_file = file,
        file_key = file_key,
    ...
    file_state.annotation_index = annotations.new_index()
    ...
    file_state.added = file.added
    file_state.removed = file.removed
    file_state.expanded = fold_state._status_folded(file_key, true) == false
    file_state.metadata = file_state.metadata or {}
    file_state.metadata.status_file = file
    file_state.metadata.file_key = file_key
...
function M._status_populate_reloaded_source_files(source_state, files, status, source_kind, source_id)
  ...
  for _, file in ipairs(files or {}) do
    file.diff_source_id = source_id
    local file_key = status_keys.file_key(source_kind, file.filename)
    local file_path = M._status_diff_file_path(file, status)
    local file_state = source.ensure_file(source_state, file_path, {
      original_path = file.original_relpath,
      status = file.git_status or file.status,
...
function M._status_source_reload_paths_loader(source_kind, source_id, status)
  return function(source_state, paths, done)
    ...
    trace.event("source.reload_paths.start", status.buf, {
      source_id = source_id,
      source_kind = source_kind,
      paths = paths,
    })
    git_backend.systemlist_async(git_backend.git_diff_command(cwd, extra_args), function(output, code, stderr)
      if code ~= 0 then
        local message = vim.trim(tostring(stderr or ""))
      ...
      for _, path in ipairs(paths or {}) do
        source.remove_file(source_state, path)
      end
      local diff_text = table.concat(output or {}, "\n")
      local files = section_map._status_files_from_diff_provider(cwd, {
      ...
      trace.event("source.reload_paths.done", status.buf, {
        source_id = source_id,
        source_kind = source_kind,
        path_count = #(paths or {}),
        file_count = #files,
...
function M._status_populate_diff_file_annotations(file_state, file)
  ...
  for _, comment in ipairs(type(status and status.review_comments) == "table" and status.review_comments or {}) do
    if size_gate._status_lazy_comment_matches_file(file, comment) then
...
function M._status_diff_hunk_annotation_ids(file_state, file, hunk)
  ...
  for _, annotation in ipairs(file_state.annotations or {}) do
    ...
    if comment and size_gate._status_lazy_comment_matches_file(file, comment) and pr_overview().hunk_contains_comment(hunk, comment) then
...
function M._status_build_diff_file_layout(file_state, file, display_hunks)
  ...
  for hunk_index, hunk in ipairs(display_hunks or {}) do
    ...
    local height = size_gate._status_lazy_hunk_estimate(file, hunk)
  ...
  file_state.body_layout = layout.new_file_body_layout(body_blocks)
...
function M._status_record_diff_file_state(file, hunks, display_hunks, entry_kind, hunk_entry_kind, file_key)
  ...
  for hunk_index, hunk in ipairs(display_hunks or {}) do
    ...
    hunk.diff_review_hunk_index = hunk.diff_review_hunk_index or hunk_index_mod.from_hunk(hunk)
