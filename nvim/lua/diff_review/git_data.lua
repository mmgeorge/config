...
local function parse_diff(diff_output, staged)
  ...
  local function flush_hunk()
    if current_file and current_hunk_lines and current_hunk_start then
      ...
      local a, r = dr()._count_stats(table.concat(current_hunk_lines, "\n"))
...
local function build_untracked_diff(filename, relpath)
  if dr()._file_contains_nul(filename) then return nil end
  ...
  if dr()._lines_contain_nul(lines) then return nil end
  local out = {
...
function M.compute_hunk_context_async(filename, line, cb)
  ...
  if ft == "" then
    ft = dr()._detect_filetype(filename)
  end
...
function M.compute_diff_syntax_async(filename, lines, cb)
  return dr()._status_perf_span("treesitter.compute_diff_syntax_async", dr()._status and dr()._status.buf or nil, {
    ...
    local ft = dr()._detect_filetype(filename, lines)
    local lang = vim.treesitter.language.get_lang(ft)
    if not lang then
      cb(nil)
      return
