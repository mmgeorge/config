...
local dependency_browser = require("forge.views.plan_review.dependency_browser")
local entity_rename = require("forge.views.plan_review.entity_rename")
local plan_fold = require("forge.views.plan_review.fold")
local plan_schema = require("forge.views.plan_review.schema")
local plan_task_model = require("forge.views.plan_review.task_model")
...
local function open_source(plan, review)
  ...
  if not ok then
    notifications.error("Failed to read plan navigation index", "ForgePlanReview")
    return
  end
  local decoded_ok, index = pcall(vim.json.decode, table.concat(source, "\n"))
  if not decoded_ok then
    notifications.error("Failed to decode plan navigation index", "ForgePlanReview")
    return
  end
  ...
  if not candidate then
    notifications.info("This plan line has no source boundary", "ForgePlanReview")
    return
  end
...
local function refresh_review_winbar(buf, status)
  ...
  keymaps.apply_view_winbar(review.win, "ForgePlanReview", "plan_review", review.command_set, status)
end

local function review_request_available()
  if not session.harness.busy then return true end
  notifications.info("A Harness request is already running", "ForgePlanReview")
  return false
end
...
local function begin_review_request(buf, status)
  ...
  require("forge.views.harness.controller").refresh_winbar()
  return true
end
...
local function attach_review_window_options(buf, win)
  ...
  local group = vim.api.nvim_create_augroup("ForgePlanReviewWindow" .. tostring(buf), { clear = true })
...
---@field task_model ForgePlanTaskModel?
---@field fold_controller ForgePlanFoldController
---@field command_set ForgeViewCommandSet?

---@param review ForgePlanReviewSession
...
local function accept(plan, review)
  ...
  client.request("plan.acceptance.begin", {
    ...
    if request_error then
      notifications.error(request_error, "ForgePlanReview")
      return
    end
    if result then
...
local function request_changes(plan, review)
  ...
  popup_window.input({ prompt = "Overall plan review comment (optional): " }, function(comment)
    ...
    client.request("plan.request_changes", {
      ...
      notifications.info("Plan revision requested", "ForgePlanReview")
      require("forge.views.harness.controller").render()
    end)
  end)
end
  ...
  entity_rename.prompt(review.task_model, review.buf, review.win, function(entity, new_name)
    ...
    client.request("plan.entity.rename", {
      ...
      if not next_plan then
        notifications.error("Entity rename returned no plan", "ForgePlanReview")
        return
      end
      session.harness.active_plan = next_plan
      close_review(review)
      notifications.info(("Renamed %s to %s"):format(entity.name, new_name), "ForgePlanReview")
...
local function commands(plan, review)
  ...
  command_set.register(set, "help", function() keymaps.show_view_help("plan_review", set, "ForgePlanReview") end)
  return set
end
...
function M.open(plan)
  ...
  local current_review = session.harness.plan_review
  if current_review and current_review.buf == existing_buf and current_review.plan.review_digest == plan.review_digest
      and existing_win and vim.api.nvim_win_is_valid(existing_win) then
    vim.api.nvim_set_current_tabpage(vim.api.nvim_win_get_tabpage(existing_win))
    vim.api.nvim_set_current_win(existing_win)
    return
  end
  if existing_buf >= 0 and vim.api.nvim_buf_is_loaded(existing_buf) and vim.bo[existing_buf].modified then
    notifications.error("Plan review has unsaved changes. Save them before opening another revision.", "ForgePlanReview")
    return
  end
  ...
  if task_model_error then notifications.error(task_model_error, "ForgePlanReview") end
...
return require("forge.views.plan_review.native_controller")
