...
local task_tree_style = require("forge.render.task_tree_style")
local uml_style = require("forge.views.plan_review.uml_style")

---@class ForgePlanTaskModel
...
---@class ForgePlanDescribedItem
---@field name string
---@field description string

---@class ForgePlanEntity: ForgePlanDescribedItem
---@field action "add"|"modify"|"remove"
---@field members ForgePlanDescribedItem[]
---@field variants ForgePlanDescribedItem[]
...
local function occupies_identifier(line, byte_col, name)
  ...
  while true do
    ...
    if first_byte and last_byte and cursor_byte >= first_byte and cursor_byte <= last_byte
...
local function file_tree_segments(model, text, target)
  ...
  if target.target_type == "file_tree_file" then
    ...
    if text:find("(new)", 1, true) then
      return task_tree_style.change("(new)", "", name, nil, "ForgeWalkthroughItemTitle")(text, 1)
    elseif text:find("(remove)", 1, true) then
      return task_tree_style.change("(remove)", "", name, nil, "ForgeWalkthroughItemTitle")(text, 1)
    end
    local renamed_from = text:match("([^%s]+)%s+→%s+" .. vim.pesc(name))
    if renamed_from then
      return task_tree_style.rename(renamed_from, name, "ForgeWalkthroughItemTitle")(text, 1)
    end
    return task_tree_style.change("", "", name, nil, "ForgeWalkthroughItemTitle")(text, 1)
...
---@param task_row_list ForgeTaskTreeRow[]
---@param width integer?
---@return table[]
...
---@class ForgePlanWorkspaceTarget
---@field name string
---@field path string
---@field line integer
...
---@param line string
---@param byte_col integer Zero-based byte column.
---@return ForgePlanWorkspaceTarget?
...
---@return ForgePlanTaskModel?, string?
