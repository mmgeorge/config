local M = {}
---@type boolean? Suppresses repository refresh while a popup returns to its origin.
M.restoring_origin = nil

local view_group = vim.api.nvim_create_augroup("ForgePopupView", { clear = false })

---@class ForgePopupOrigin
---@field win integer
---@field mode string

---@class ForgePopupWindowOptions
...
---@param options ForgePopupWindowOptions
---@return integer, integer
function M.open(options)
  ...
  vim.bo[buf].filetype = options.filetype or "ForgePopup"
  ...
  vim.api.nvim_create_autocmd({ "CursorMoved", "WinScrolled" }, {
    ...
    desc = "Keep Forge popup content inside its rendered lines",
  })
  vim.api.nvim_create_autocmd("WinClosed", {
    ...
    desc = "Restore the window and mode that opened a Forge popup",
  })
  if options.enter ~= false then vim.api.nvim_set_current_win(popup_win) end
...
function M.close(win, restore)
  ...
  local previous = M.restoring_origin
  M.restoring_origin = true
  local ok, failure = pcall(function()
    if vim.api.nvim_win_is_valid(win) then vim.api.nvim_win_close(win, true) end
    if restore ~= false then M.restore_origin(origin) end
  end)
  M.restoring_origin = previous
...
function M.set_cursor_hidden(win, hidden, base)
  ...
  highlight_list = vim.tbl_filter(function(entry)
    ...
    return entry ~= "Cursor:ForgeHiddenCursor"
  end, highlight_list)
  if hidden then highlight_list[#highlight_list + 1] = "Cursor:ForgeHiddenCursor" end
