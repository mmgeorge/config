--- Owns diff-scope performance tracing for render, ForgeStatus, PR, and shared UI paths. Builds
--- per-buffer payloads and writes structured events through infra/perf without depending on views.
---@class ForgePerfTraceModule
local M = {}
...
function M.payload(buf, extra)
  ...
  local payload = perf.payload(extra or {})
  payload.buf = buf
...
function M.span(event, buf, extra, callback)
  if not perf.enabled("diff") then return callback() end
  local payload = M.payload(buf, extra)
