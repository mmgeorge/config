--- Commit flow for Forge, modelled on Neogit.
---
--- Forge's commit action runs `git commit` as a job with a headless "fake editor" wired to
...
local function runtimepath_root()
  ...
  -- Strip /lua/forge/... to the config root holding lua/, depth-agnostic so the
  -- module can move within the plugin without breaking the headless editor's require path.
  return (source:gsub("/lua/forge/.*$", ""))
end
...
local function remote_editor_cmd()
  ...
  local lua_cmd = vim.fn.shellescape("lua require('forge.integrations.commit').client()")
  return table.concat({
...
function M.client()
  local ok, err = pcall(function()
    ...
    local code = ("lua require('forge.integrations.commit').editor(%q, %q)"):format(target, client_addr)
...
---@param buf integer
---@param data string?
local function append_text(buf, data)
  if not data or data == "" then return end
  append(buf, vim.split(data, "\n", { plain = true }))
end
...
function M.commit(opts)
  ...
  if M._admission_pending or M._active then
    vim.notify("A commit is already in progress", vim.log.levels.WARN, { title = "Forge commit" })
    return
  end
  ...
  local function resolve_root(command, system_options, callback)
    if opts.workspace then
      vim.schedule(function() callback({ code = 0, stdout = opts.workspace }) end)
      return
    end
    return vim.system(command, system_options, callback)
  end
  local ok, root_process_or_error = pcall(resolve_root, { "git", "rev-parse", "--show-toplevel" }, {
    text = true,
    stdout = true,
    stderr = true,
  }, function(result)
