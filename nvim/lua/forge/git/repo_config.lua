...
--- Reads and parses the `.forge.json` repository configuration file.
---@param cwd string Repository root directory path.
---@return ForgeRepoConfig config Decoded repository configuration table.
function M.read(cwd)
  local path = (cwd:gsub("[/\\]+$", "")) .. "/.forge.json"
  local content
  if M.reader then
