...
---@alias ForgePathStatusKind "ordinary"|"renamed"|"copied"|"unmerged"|"untracked"
---@alias ForgePathStatusSnapshotSource "status"|"unstaged_diff"|"staged_diff"|"unstaged_added_numstat"|"staged_added_numstat"
---@alias ForgePreviewState "loaded"|"unloaded"|"loading"|"omitted"|"error"
---@alias ForgePreviewSource "diff"|"worktree_added"|"index_added"|"index_deleted"|"head_deleted"

---@class ForgePathPreview
---@field state ForgePreviewState
---@field source ForgePreviewSource
---@field added integer
...
---@class ForgePathStatusRecord
---@field kind ForgePathStatusKind
...
---@class ForgePathStatusFileSnapshot
---@field path string repository-relative current path
---@field abs_file string absolute current path
---@field original_path? string repository-relative source path for a rename or copy
---@field status_record ForgePathStatusRecord
---@field unstaged_hunk_list ForgeHunk[]
---@field staged_hunk_list ForgeHunk[]
...
---@field unstaged_preview ForgePathPreview
---@field staged_preview ForgePathPreview

---@class ForgePathStatusCommandFailure
---@field source ForgePathStatusSnapshotSource
---@field command ForgeGitCommand
---@field code integer
...
---@class ForgePathStatusSnapshotError
---@field kind "input"|"command"|"parse"
---@field message string
---@field source? ForgePathStatusSnapshotSource
---@field failure_list? ForgePathStatusCommandFailure[]

---@class ForgePathStatusSnapshot
---@field root string
...
---@field status_record_list ForgePathStatusRecord[]
---@field status_record_by_path table<string, ForgePathStatusRecord>
---@field file_list ForgePathStatusFileSnapshot[]
---@field file_by_path table<string, ForgePathStatusFileSnapshot>
...
---@class ForgePathStatusSnapshotModule
local M = {}
...
---@return ForgePathStatusSnapshotError? error Validation error descriptor, or nil.
...
---@param kind ForgePathStatusKind Record category descriptor.
...
---@return ForgePathStatusRecord record Populated path status descriptor table.
...
---@return ForgePathStatusRecord[]? record_list Array of parsed path records, or nil on error.
...
---@param command ForgeGitCommand Command arguments array.
...
---@return table<ForgePathStatusSnapshotSource, ForgeGitCommand> commands Map of commands by source key.
...
---@param hunk_list ForgeHunk[] Array of parsed diff hunks.
...
---@return ForgePathStatusRecord record Inferred path status record.
...
---@return table<string, ForgeNumstat>? stat_by_path Map of line statistics by relative path, or nil on error.
...
---@param content string Raw file content string.
---@return ForgeNumstat stat Calculated line addition and binary flags.
...
---@param callback fun(stat_by_path?: table<string, ForgeNumstat>, error?: ForgePathStatusSnapshotError) Completion callback function.
...
---@param untracked_stat_by_path table<string, ForgeNumstat> Untracked line statistics map.
---@return ForgePathStatusSnapshot? snapshot Compiled path status snapshot, or nil on error.
---@return ForgePathStatusSnapshotError? error Parse error descriptor, or nil.
local function build_snapshot(
  ...
  ---@type table<string, ForgeHunk[]>
  local unstaged_hunk_by_path = {}
  ---@type table<string, ForgeHunk[]>
  local staged_hunk_by_path = {}

  ---@param hunk_list ForgeHunk[]
  ---@param hunk_by_path table<string, ForgeHunk[]>
  ...
  for _, path in ipairs(path_order) do
    ...
    ---@type ForgePathStatusFileSnapshot
    local file_snapshot = {
      path = path,
      abs_file = abs_file,
...
---@param staged_added_numstat_output? string Raw staged added numstat output string.
---@return ForgePathStatusSnapshot? snapshot Compiled path status snapshot, or nil on error.
---@return ForgePathStatusSnapshotError? error Snapshot construction error descriptor, or nil.
...
---@param source ForgePathStatusSnapshotSource Snapshot phase key.
---@param command ForgeGitCommand Command arguments array.
---@param result ForgeGitAsyncResult Process result table.
---@return ForgePathStatusCommandFailure failure Formatted failure descriptor.
...
---@param callback fun(snapshot?: ForgePathStatusSnapshot, error?: ForgePathStatusSnapshotError) Completion callback function.
function M.collect_async(root, path_list, callback)
  ...
  ---@type table<ForgePathStatusSnapshotSource, ForgeGitAsyncResult>
  local result_by_source = {}
  local pending = 5
  local finished_source = {}
  local callback_finished = false

  ---@param snapshot? ForgePathStatusSnapshot
  ---@param snapshot_error? ForgePathStatusSnapshotError
  ...
  ---@param source ForgePathStatusSnapshotSource
  ---@param result ForgeGitAsyncResult
