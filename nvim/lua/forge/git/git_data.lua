...
local diff_buffer = require("forge.views.diff_buffer")
...
---@type fun(snapshot_error: ForgePathStatusSnapshotError)
...
---@return ForgePathStatusSnapshotModule
local function status_snapshot()
  return (require("forge.git.status_snapshot"))
end
...
---@param file ForgeStatusFile Status file record.
...
function M._status_file_change_label(file)
  ...
  if file.untracked or status:sub(1, 1) == "a" or status == "added" or status == "new" then
    return "New", "ForgeStatusFileNew"
  end
  if status_file_is_deleted(file) then
    return "Removed", "ForgeStatusFileDeleted"
  end
  return "Modified", "ForgeStatusFileModified"
end

---@class ForgeStatusFileStatSegment
...
---@return ForgeStatusFileStatSegment[] segments Highlight spans mapping columns to groups.
function M._status_file_stat_text_and_segments(file)
  ...
  return added_text .. " " .. removed_text, {
    { start_col = 0, end_col = #added_text, hl_group = "ForgeAddRange" },
    { start_col = #added_text + 1, end_col = #added_text + 1 + #removed_text, hl_group = "ForgeDeleteRange" },
  }
end
...
---@param hunks ForgeHunk[] hunks for one file, each with .pos, .staged, .diff
...
---@param cwd string
---@param callback fun(snapshot?: ForgePathStatusSnapshot, error?: ForgePathStatusSnapshotError)
...
---@param cb fun(context?: ForgeHunkTreeSitterContext|string) Callback receiving computed scope context.
...
---@param cb fun(syntax?: ForgeTreeSitterSyntax) Callback receiving parsed Tree-sitter syntax tree.
...
---@param cb fun(syntax?: ForgeTreeSitterSyntax) Callback receiving parsed Tree-sitter syntax data.
...
---@param file_snapshot ForgePathStatusFileSnapshot
---@param staged boolean
---@return ForgeHunk
...
---@return ForgeHunk[] all_hunk_list
---@return ForgePathStatusFileSnapshot[] untracked_file_list
...
---@param untracked_file_list ForgePathStatusFileSnapshot[]
...
---@param callback fun(item_list?: table[], error?: ForgePathStatusSnapshotError, snapshot?: ForgePathStatusSnapshot) Completion callback.
