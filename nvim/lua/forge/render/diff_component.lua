...
local buffer_accumulator = require("forge.render.buffer_accumulator")
...
function M.file_segment_list(file, indent)
  ...
  segment_list[#segment_list + 1] = { file.relpath, "ForgeStatusPath" }
  segment_list[#segment_list + 1] = { " " }
  for _, segment in ipairs(buffer_accumulator.highlighted_text_segments(stats, stat_segment_list)) do
...
function M.append_file_header(state, file, entry, indent)
  ...
  return buffer_accumulator.add_segment_line(state, segment_list, entry), segment_list
end
...
function M.append_rows(state, row_list, entry, indent, include)
  ...
  for _, row in ipairs(row_list or {}) do
    if not include or include(row) then
      ...
      local line = buffer_accumulator.add_fancy_row(state, row, row_entry, indent)
      result.last_line = line
      if row.forge_hunk_header then result.fold_text = state.lines[line] end
    end
  end
  return result
end
