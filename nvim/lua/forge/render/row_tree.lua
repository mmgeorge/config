...
local layout = require("forge.render.layout")

---@class ForgeRowTreeNode
---@field key string
---@field row_count integer
---@field metadata table?

---@class ForgeRowTree
---@field node_list ForgeRowTreeNode[]
...
---@field layout ForgeViewLayout
---@field node_by_key table<string, ForgeRowTreeNode>
---@field revision integer

---@class ForgeRowTreeModule
local M = {}
...
---@param node ForgeRowTreeNode
---@param item_index integer
---@param row_start integer
---@return ForgeLayoutSpan
...
---@param tree ForgeRowTree Target row tree structure.
...
---@return ForgeRowTree tree Initialized row tree structure.
function M.new(opts)
  opts = opts or {}
  local tree = {
...
---@return ForgeRowTree tree Constructed row tree instance.
function M.from_status_state(opts)
  ...
  while line_number <= #line_list do
    ...
    local block = type(line_value) == "table" and line_value.__forge_lazy_hunk and line_value.block or nil
    if block then
      local consumed = 1
      while line_number + consumed <= #line_list do
        local next_value = line_list[line_number + consumed]
        local next_block = type(next_value) == "table" and next_value.__forge_lazy_hunk and next_value.block or nil
...
---@return ForgeLayoutSpan? span Matching layout span, or nil.
...
---@param node_list ForgeRowTreeNode[] Array of replacement nodes.
...
---@return ForgeLayoutSpan[] spans Array of intersecting layout spans.
...
---@return ForgeLayoutSpan? span Matching layout span, or nil.
---@return ForgeRowTreeNode? node Matching row tree node, or nil.
