...
---@class ForgeHunkModelModule
local M = {}

local diff_render = require("forge.render.diff_render")
-- syntax_engine edge kept lazy to avoid a load-time cycle.
local function syntax_engine() return require("forge.render.syntax_engine") end
local diff_parse = require("forge.render.diff_parse")
...
---@param context ForgeHunkTreeSitterContext|string? Syntax context descriptor or name.
---@return ForgeHunkBoundaryContext? boundary Innermost ancestor boundary descriptor, or nil.
...
---@param hunk ForgeParsedHunk Parsed diff hunk descriptor.
---@return fun(parsed_line: ForgeParsedHunkLine): boolean filter Filter predicate returning true for visible lines.
function M.render_line_filter(hunk)
  ...
  for _, parsed_line in ipairs(hunk.lines or {}) do
    if parsed_line.prefix == "+" or parsed_line.prefix == "-" then
      local position = parsed_line.position
      if position then
        first_changed_position = math.min(first_changed_position or position, position)
        last_changed_position = math.max(last_changed_position or position, position)
      end
    end
  end
...
---@param parsed_lines ForgeParsedHunkLine[] Array of parsed hunk lines.
---@param include_line? fun(parsed_line: ForgeParsedHunkLine): boolean Optional line filter predicate.
...
---@return ForgeParsedHunkLine line Primary parsed diff line.
...
---@param item table Render item descriptor table.
---@return ForgeParsedHunkLine[] lines Array of backing diff lines.
...
---@param parsed_line ForgeParsedHunkLine? Parsed diff line.
...
---@param line_by_position table<integer, integer> Mapping from hunk position to current line number.
---@param context_for_line fun(line: integer): ForgeHunkTreeSitterContext|string? Resolves Tree-sitter context for a line number.
---@return ForgeHunkChangeRegion[] regions Array of change region descriptor tables.
function M.change_regions(render_items, line_by_position, context_for_line)
  local regions = {}
  local current_region = nil ---@type ForgeHunkChangeRegion?
...
---@param region { added?: integer, removed?: integer } Change region descriptor table.
---@param fallback_hunk? ForgeParsedHunk Optional fallback parsed hunk for missing counts.
...
function M.virtual_header_parts(region, fallback_hunk)
  ...
  return {
    { "@@ ", "ForgeHunkHeader" },
    { ("+%d"):format(added or 0), "ForgeAddRange" },
    { " ", "ForgeHunkHeader" },
    { ("-%d"):format(removed or 0), "ForgeDeleteRange" },
  }
end

--- Tests whether a hidden context line is a closing delimiter right after modified lines.
---@param parsed_line ForgeParsedHunkLine Parsed hunk line to inspect.
---@param previous_visible_changed boolean True if the previous visible line was changed.
---@param context ForgeHunkTreeSitterContext|string? Syntax context descriptor.
...
---@param parsed_line ForgeParsedHunkLine Parsed hunk line.
...
---@param parsed_lines ForgeParsedHunkLine[]? Array of parsed diff lines.
---@param file string File path identifier.
...
---@param gutter ForgeGutterSpec Gutter layout configuration.
---@param file string File path identifier.
---@param syntax? ForgeTreeSitterSyntax Syntax engine state for line highlights.
...
function M.replacement_row(replacement, gutter, file, syntax, syntax_row)
  ...
  local sign_hl = "ForgeModifyLineNr"
  local line_hl = "ForgeModifyBg"
  local row = {
    forge_bg_hl = line_hl,
    forge_inline_highlights = replacement.inline_spans or {},
  }
  row[#row + 1] = {
    "",
    nil,
...
---@param syntax? ForgeTreeSitterSyntax Optional syntax engine state.
...
function M.context_padding_row(line_number, text, gutter, file, syntax, old_line, new_line)
  local row = { forge_context_padding = true }
...
---@return ForgeHunkContextPaddingLine[] padding_lines Array of context padding line descriptor tables.
...
---@param hunks ForgeParsedHunk[] Array of parsed diff hunks.
...
---@param padding_lines ForgeHunkContextPaddingLine[] Array of padding line records.
---@param block ForgeParsedBlock Parent parsed hunk block descriptor.
---@return ForgeHunkContextPaddingLine[] padding_lines Annotated array of padding line records.
...
---@param region ForgeHunkChangeRegion Target change region.
---@param render_items table[] Array of render items.
---@param include_render_line fun(parsed_line: ForgeParsedHunkLine): boolean Line filter predicate.
---@return table<string, boolean> visible Set of visible code line strings.
...
---@param region ForgeHunkChangeRegion Target change region.
---@param before_padding_lines ForgeHunkContextPaddingLine[] Leading padding lines.
---@param after_padding_lines ForgeHunkContextPaddingLine[] Trailing padding lines.
...
---@param previous_plan ForgeHunkRenderPlan Earlier render plan in file order.
---@param next_plan ForgeHunkRenderPlan Subsequent render plan.
...
---@param left ForgeHunkTreeSitterContext|string? First context descriptor.
---@param right ForgeHunkTreeSitterContext|string? Second context descriptor.
...
---@param block ForgeParsedBlock Parent hunk block descriptor.
...
---@param plan ForgeHunkRenderPlan Target render plan.
---@param raw_context ForgeHunkTreeSitterContext|string? Syntax context descriptor.
...
---@param previous_plan ForgeHunkRenderPlan Earlier render plan.
---@param next_plan ForgeHunkRenderPlan Subsequent render plan.
