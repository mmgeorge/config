...
local util = require("forge.infra.util")
local trace = require("forge.infra.perf_trace")
-- hunk_model edge kept lazy to avoid a load-time cycle.
local function hunk_model() return require("forge.render.hunk_model") end
local intraline_diff = require("forge.render.intraline_diff")
-- diff_buffer edge kept lazy to avoid a load-time cycle.
local function diff_buffer() return require("forge.views.diff_buffer") end
...
---@return ForgeGutterSpec spec Default gutter layout descriptor.
...
local function hunk_gutter_chunks(gutter, old_line, new_line, sign, sign_hl, line_hl, changed_line_hl)
  ...
  if old_line then
    if sign == "-" then
      old_hl = changed_line_hl or sign_hl or "ForgeDeleteLineNr"
    elseif sign == "~" then
      old_hl = changed_line_hl or sign_hl or "ForgeModifyLineNr"
    else
      old_hl = "ForgeContextLineNr"
    end
  end
  local new_hl = line_hl
  if new_line then
    if sign == "+" then
      new_hl = changed_line_hl or sign_hl or "ForgeAddLineNr"
    elseif sign == "~" then
      new_hl = changed_line_hl or sign_hl or "ForgeModifyLineNr"
    else
      new_hl = "ForgeContextLineNr"
    end
  end
  local chunks = {}
...
local function hunk_boundary_row(text, segments, line_number, gutter, file, old_line, new_line)
  ...
  if segments and #segments > 0 then
    ...
    row[#row + 1] = { text, "ForgeHunkBoundary" }
  end
  return row
end
...
local function hunk_header_row(header_parts)
  local row = { forge_hunk_header = true }
...
---@param parsed_line ForgeParsedHunkLine Parsed hunk line record.
---@param gutter ForgeGutterSpec? Gutter layout configuration.
---@param file string? Optional file path identifier.
---@param syntax? ForgeTreeSitterSyntax Optional syntax highlight state.
...
local function hunk_body_row(parsed_line, gutter, file, syntax, syntax_row)
  ...
  if parsed_line.prefix == "+" then
    sign_hl = "ForgeAddLineNr"
    line_hl = "ForgeAddBg"
  elseif parsed_line.prefix == "-" then
    sign_hl = "ForgeDeleteLineNr"
    line_hl = "ForgeDeleteBg"
  end

  local row = { forge_bg_hl = line_hl }
  local diff_meta = {
    diff = hunk_model().diff_line_meta(parsed_line, file or ""),
  }
...
---@param left ForgeGutterSpec? First gutter specification.
---@param right ForgeGutterSpec? Second gutter specification.
---@return ForgeGutterSpec spec Combined gutter specification.
...
---@param group ForgeHunkDisplayGroup Target display group structure.
---@param plan ForgeHunkRenderPlan Render plan to append.
local function add_plan_to_hunk_display_group(group, plan)
  ...
  if plan.display_start then
    group.display_start = group.display_start and math.min(group.display_start, plan.display_start) or plan.display_start
  end
  if plan.display_end then
    group.display_end = group.display_end and math.max(group.display_end, plan.display_end) or plan.display_end
  end
...
---@param plans ForgeHunkRenderPlan[] Array of render plans.
---@return ForgeHunkDisplayGroup[] groups Array of display group tables.
local function merge_hunk_render_plans(plans)
  local groups = {}
  local current_group = nil ---@type ForgeHunkDisplayGroup?
...
---@param hunk_staged boolean|boolean[]? True if hunks are staged.
...
local function build_fancy_diff_rows(diff_text, hunk_staged, filename, context_callback_key, on_context_update, opts)
  ...
  ---@class ForgeFancyDiffRows : table<integer, table>
  ---@field forge_syntax_pending boolean
  ---@field forge_context_pending boolean
  local ret = {} ---@type ForgeFancyDiffRows
  local hunk_idx = 0
  ...
  if filename then
    ...
    if debug_context then
      trace.event("build_fancy_diff_rows.syntax", session.status and session.status.buf or nil, {
  ...
  local plans = {} ---@type ForgeHunkRenderPlan[]
          ...
          if not suppress_start_boundary
            ...
            if not adjacent_to_next then
              if boundary.selected then
                ...
                if hidden_row and hidden_line then
  ...
  ret.forge_syntax_pending = syntax_pending
  ret.forge_context_pending = context_pending
  return ret
end
    ...
    if row.forge_bg_hl then
      ...
      extmarks[#extmarks + 1] = {
        ...
        opts = {
          end_row = row_index,
          end_col = 0,
          hl_group = row.forge_bg_hl,
          priority = 60,
          hl_eol = true,
        },
      }
    end
    for _, inline_highlight in ipairs(row.forge_inline_highlights or {}) do
      highlights[#highlights + 1] = {
        line = row_index,
