...
---@class ForgeHunkIndex
---@field hunk table
---@field header_line_list string[]
---@field section_list ForgeHunkIndexSection[]
---@field body_row_list ForgeHunkIndexBodyRow[]

---@class ForgeHunkIndexModule
local M = {}
...
---@return ForgeHunkIndex index Initialized hunk index table.
...
---@param hunk table Raw diff hunk descriptor table.
---@return ForgeHunkIndex index Existing or newly attached hunk index.
function M.ensure(hunk)
  if hunk and hunk.forge_hunk_index then return hunk.forge_hunk_index end
  local index = M.from_hunk(hunk)
  if hunk then hunk.forge_hunk_index = index end
  return index
end

--- Extracts a windowed chunk of lines from the indexed hunk, formatting a valid sub-hunk.
---@param index ForgeHunkIndex Target hunk index.
