...
local diff_component = require("forge.render.diff_component")
    ...
    if expanded[file_key] then
      for hunk_index, raw_hunk in ipairs(block.hunks or {}) do
        ...
        diff_component.append_rows(result, row_list, function(row)
          return {
            kind = row.forge_hunk_header and "hunk" or "diff",
            file_path = file_path,
            interaction = options.interaction,
            expand_key = row.forge_hunk_header and row_key or nil,
          }
        end, indent, function(row)
          if row.forge_hunk_header then return true end
