...
local hunk_model = require("forge.render.hunk_model")
...
local function status_diff_hunks_for_file(file) return file and (file.hunks or file.diff_hunks) or {} end
...
---@return ForgeHunkTreeSitterContext|string|table? context Cached context record or pending table.
...
---@return ForgeTreeSitterSyntax|table? syntax Cached syntax state record or pending table.
...
local function mark_syntax_scratch_buffer(buf, filetype)
  vim.b[buf].forge_syntax_scratch = true
  vim.b[buf].forge_syntax_filetype = filetype or ""
end
...
local function syntax_buffer_filetype(buf, filename)
  local filetype = vim.b[buf].forge_syntax_filetype
...
---@param query any? Highlight query instance.
...
---@param buf integer Source buffer number.
---@param query any Scope query instance.
---@param highlight_query any? Syntax highlight query instance.
---@param trees any[] Array of parsed Tree-sitter trees.
---@param target integer Zero-based target line number.
---@return ForgeHunkTreeSitterContext? context Enclosing scope context structure, or nil.
...
---@param on_update fun(context: ForgeHunkTreeSitterContext?)? Async callback triggered upon computation completion.
---@return ForgeHunkTreeSitterContext|string? context Cached context if ready, or nil if pending.
...
---@param filename string Target file path.
---@param callback_key string Unique cache key for listener callback registration.
---@param on_update fun(syntax: ForgeTreeSitterSyntax?)? Async callback triggered upon computation completion.
---@return ForgeTreeSitterSyntax? syntax Cached syntax state if ready, or nil if pending.
...
---@param context ForgeHunkTreeSitterContext|string? Context descriptor or string.
...
---@param context ForgeHunkTreeSitterContext|string? Context descriptor.
...
---@param on_update fun(syntax: ForgeTreeSitterSyntax?)? Async completion callback.
---@return ForgeTreeSitterSyntax? syntax Cached syntax state if ready, or nil if pending.
...
local function cached_diff_syntax(filename, diff_text, side, callback_key, on_update)
  ...
  return trace.span("syntax.cached_diff_syntax", status_buf, {
    ...
    local cache_root = ts_diff_syntax_cache
    local scope = filename .. "\0" .. side
    local source_cache = cache_root[scope]
    if not source_cache then source_cache = {} cache_root[scope] = source_cache end
    local cache_key = diff_text or ""
    local cached = source_cache[cache_key]
    if cached == false then return nil, false end
    if type(cached) == "table" and not cached.pending then return cached, false end
    if type(cached) == "table" and cached.pending then
      if on_update then cached.callbacks[callback_key] = on_update end
    ...
    trace.span("syntax.cached_diff_syntax.compute", status_buf, {
      ...
      git_data().compute_diff_syntax_async(filename, lines, function(syntax)
        if ts_diff_syntax_cache ~= cache_root then return end
        local pending = ts_diff_syntax_cache and source_cache[cache_key]
        local callbacks = type(pending) == "table" and pending.callbacks or {}
        source_cache[cache_key] = syntax or false
        for _, callback in pairs(callbacks) do
          local ok, err = pcall(callback, syntax)
...
local function cached_old_file_syntax(filename, diff_text, callback_key, on_update)
  ...
  return trace.span("syntax.cached_old_file_syntax", status_buf, {
    ...
    ts_diff_syntax_cache = ts_diff_syntax_cache or {}
    local cache_root = ts_diff_syntax_cache
    local scope = filename .. "\0" .. "old-file"
    local source_cache = cache_root[scope]
    if not source_cache then source_cache = {} cache_root[scope] = source_cache end
    local cache_key = diff_text or ""
    local cached = source_cache[cache_key]
...
---@param hunk_staged boolean|boolean[]? Staged flag or array of staged flags per hunk.
...
local function diff_uses_file_syntax(hunk_staged, opts)
  ...
  for _, staged in ipairs(type(hunk_staged) == "table" and hunk_staged or {}) do
