...
---@class ForgeTaskTreeStyleTerm
---@field text string
---@field hl string?

---@param line string
---@param term_list ForgeTaskTreeStyleTerm[]
---@return table[]
...
function M.task(title)
  return function(line, line_index)
    ...
    return ordered_segments(line, {
      { text = title, hl = "ForgeWalkthroughItemTitle" },
    })
  end
end
...
function M.rename(renamed_from, name, target_highlight)
  return function(line, line_index)
    if line_index ~= 1 then return { { line } } end
    return ordered_segments(line, {
      { text = renamed_from, hl = "ForgeWalkthroughActionRename" },
      { text = "→", hl = "ForgeWalkthroughActionRename" },
...
function M.file(path)
  return function(line, line_index)
    if line_index ~= 1 then return { { line } } end
    return ordered_segments(line, {
      { text = "file", hl = "ForgeFileKeyword" },
      { text = path, hl = "ForgeWalkthroughItemTitle" },
    })
  end
end
