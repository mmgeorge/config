...
local ok, err = xpcall(function()
  ...
  issue_index_builder._set_artifact_root_for_test(vim.fs.joinpath(build_root, "artifacts"))
  local build_count = 0
  issue_index_builder._set_runner_for_test(function(command, opts, callback)
    ...
    assert_equals(command[5], "--target-dir", "builder should isolate Cargo output from launched executables")
    assert_equals(command[6], vim.fs.joinpath(issue_index_builder.artifact_root(), "build"), "builder target dir mismatch")
    assert_equals(command[7], "--release", "builder should build release binary")
    ...
    assert_equals(vim.fn.writefile({ "fake binary " .. string.rep("x", build_count) }, issue_index_builder.release_binary_path()), 0, "fake binary write failed")
  ...
  assert_true(build_result.path ~= issue_index_builder.release_binary_path(), "builder must not launch Cargo's replaceable output")
  assert_equals(build_result.path, issue_index_builder.deployed_binary_path(), "builder returned wrong deployed binary path")
  assert_true(vim.uv.fs_stat(build_result.path) ~= nil, "builder did not stage the deployed binary")
  ...
  assert_equals(cached_build_result.path, build_result.path, "cached builder should reuse the immutable deployment")
  assert_equals(build_count, 1, "builder should skip cargo when release binary is current")

  assert_equals(vim.fn.delete(issue_index_builder.release_binary_path()), 0, "test release binary delete failed")
  local rebuilt_result
  issue_index_builder.ensure(function(result)
    rebuilt_result = result
  end)
  assert_true(vim.wait(1000, function() return rebuilt_result ~= nil end, 10), "rebuilt builder callback did not run")
  assert_true(rebuilt_result.ok, rebuilt_result.message)
  assert_equals(build_count, 2, "missing Cargo output should trigger a rebuild")
  assert_true(rebuilt_result.path ~= build_result.path, "a rebuild should publish a new immutable deployment")
  assert_true(vim.uv.fs_stat(build_result.path) ~= nil, "a rebuild must preserve the previously launched deployment")
  issue_index_builder._reset_for_test()
