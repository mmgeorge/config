...
local ok, failure = pcall(function()
  forge.setup({ walkthrough_inventory = "sem", harness = { backend = "mock" } })
  ...
  assert_equals(active_picker.spec.page_list[1].wrap_selection, false,
    "session selection should stop at either end")
  assert_equals(vim.api.nvim_win_get_config(active_picker.win).title_pos, "left")
  assert_equals(active_picker.spec.page_list[1].header_right, "Search: Repository | Open: Current Tab")
  ...
  assert_equals(vim.api.nvim_get_current_win(), active_picker.win, "session picker should start on options")
  assert_equals(vim.api.nvim_get_mode().mode, "n", "session picker should start in normal mode")
  assert_equals(active_picker.frame.search_start, nil, "normal-mode session picker should hide the search row")
  assert_equals(active_picker.search_win, nil, "normal-mode session picker should not open search input")
  local search_mapping = vim.fn.maparg("/", "n", false, true)
  assert_true(search_mapping.callback ~= nil, "session picker should retain search access")
  search_mapping.callback()
  assert_true(active_picker.frame.search_start ~= nil, "entering search should reveal its row")
  assert_equals(vim.api.nvim_get_current_win(), active_picker.search_win, "/ should focus session search")
  vim.cmd("stopinsert")
  vim.api.nvim_exec_autocmds("InsertLeave", { buffer = active_picker.search_buf })
  assert_true(vim.wait(1000, function() return active_picker.search_win == nil end, 10),
    "leaving an empty search should close its input")
  assert_equals(active_picker.frame.search_start, nil, "empty search row should disappear in normal mode")
  assert_equals(vim.api.nvim_get_current_win(), active_picker.win, "empty search should return focus to options")
  search_mapping.callback()
  assert_true(active_picker.search_win ~= nil, "search should reopen after the empty row disappears")
  ...
  assert_true(active_picker.search_win ~= nil and active_picker.frame.search_start ~= nil,
    "leaving a nonempty search should preserve the filter and input")
  ...
  assert_true(picker._state_for_test().spec.page_list[1].header_right:find("Search: All", 1, true) ~= nil)
