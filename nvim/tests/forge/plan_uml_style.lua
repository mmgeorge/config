...
assert_equals(
  vim.api.nvim_get_hl(0, { name = "ForgeDependencyName", link = false }).fg,
  tonumber("9dffe7", 16),
  "dependency names should use the shared teal palette color"
)
assert_equals(
  vim.api.nvim_get_hl(0, { name = "ForgePlanMetadata", link = false }).fg,
...
local task_model, load_error = require("forge.views.plan_review.task_model").load(working_path)
