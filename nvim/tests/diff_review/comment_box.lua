...
comment_box.render_box(state, descriptor, 1)
comment_box.render_box(state, { heading = " second ", body_lines = { "second body" } }, 1)
...
assert_true(comment_box.refresh_buffer(state), "width change did not request a compact-box layout render")
assert_true(not comment_box.refresh_buffer(state), "unchanged width requested a duplicate layout render")
