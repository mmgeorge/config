...
local function is_diff_review_git_command(command)
  local key = command_key(command)
  return key == "git\trev-parse\t--show-toplevel" or key:sub(1, #("git\t-C\t" .. root .. "\t")) == "git\t-C\t" .. root .. "\t"
end
...
local function run()
  ...
  for _, call in ipairs(calls) do
    if not is_diff_review_git_command(call.command) then goto continue end
    checked = checked + 1
