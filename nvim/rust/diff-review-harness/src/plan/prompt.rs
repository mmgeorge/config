...
- Set `exclusive_owner_entity` only when a type intentionally forms a private implementation detail used exclusively by one parent. Do not infer exclusivity merely from one current caller.
...
- Harness renders every flow as a vertical pipeline with actions and child operations on the left and planned paths or external owners in one aligned right column.
- Order top-level steps in conceptual execution order from the actual entry point to the observable consumer or effect.
- Give every step a concise action and its planned or external entity target. Put the value, event, or result crossing the boundary in `value_to_next`. Use `{"kind":"type","name":"TypeName"}` for a named type and `{"kind":"text","text":"observable value"}` for all other values, events, results, and effects. A final step may use `value_to_next` for its observable result, which the projection closes with `└─` instead of implying another step.
- Put independent child work owned by one composite step in `operations`. Give each operation its own action, planned or external target, and optional result. Operations are one level deep and their array order controls reviewer presentation rather than causal dependency.
- Keep causally ordered work as separate top-level steps. Do not use operations when one child consumes another child's result.
...
- Treat each task file as a concrete source-file boundary. Always use its repository-relative path. Never substitute a module, package, directory, or friendly label for the path.
...
- Keep contract operations on their contract, preserve retained-state ownership notation, and name the intentional owner of every exclusive child type.
      ...
      "steps": [
        {
          ...
          "value_to_next": {"kind": "type", "name": "DraftChange"}
        },
        {
          ...
          "operations": [
            {
              "action": "index_by_document()",
              "target": {"kind": "planned_entity", "entity": "DraftCache"},
              "result": "DraftId"
            },
            {
              "action": "persist()",
              "target": {"kind": "external_entity", "entity": "draft storage"},
              "result": null
            }
          ]
        }
...
mod test {
    ...
    fn planning_contract_requires_structured_submission_without_native_mode() {
        ...
        assert!(prompt.contains("actual stdout or stderr line structure with concrete sample values"));
        ...
        assert!(prompt.contains("renders every flow as a vertical pipeline"));
        ...
        assert!(prompt.contains("ownership boundary, architectural risk, or independent review reason"));
        assert!(prompt.contains(r#"{"kind":"type","name":"TypeName"}"#));
        assert!(prompt.contains(r#"{"kind":"text","text":"observable value"}"#));
        assert!(prompt.contains("final step may use `value_to_next` for its observable result"));
        assert!(prompt.contains("one level deep"));
        assert!(prompt.contains("rather than causal dependency"));
        ...
        assert!(prompt.contains("flow-step `operations` are always complete replacement arrays"));
