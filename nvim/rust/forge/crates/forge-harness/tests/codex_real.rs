...
async fn asks_for_feedback_then_creates_a_plan_without_native_collaboration_mode() {
    ...
    assert_eq!(
        paused_plan.state,
        forge_harness::plan::PlanState::AwaitingInput
    );
    ...
    assert_eq!(
        ...
        forge_harness::plan::PlanState::AwaitingReview
    );
    ...
    assert!(snapshot.timeline.iter().any(|entry| matches!(
        entry,
        forge_harness::timeline::TimelineEntry::Status {
            status: forge_harness::session::state_machine::SessionPhase::AwaitingPlanReview { .. },
            ..
        }
    )));
...
async fn geoparquet_prompt_reaches_plan_review_after_scope_feedback() {
    ...
    for _ in 0..3 {
        ...
        match plan.state {
            forge_harness::plan::PlanState::AwaitingReview => break,
            forge_harness::plan::PlanState::AwaitingInput => {
                let question = plan
                    .elicitation
                    .as_ref()
    ...
    assert_eq!(plan.state, forge_harness::plan::PlanState::AwaitingReview);
