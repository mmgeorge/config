...
impl CodexJsonRpc {
    ...
    async fn write_encoded_message(&mut self, message: &Value, encoded: String) -> Result<()> {
        self.trace
            .record_ref(&self.session_id, "model.sent", &message);
        self.socket
    ...
    async fn provider_request_result(
        ...
        match method {
            ...
            "item/tool/call" => {
                ...
                if let Some(invocation) = control_invocation(method, params)? {
                    ...
                    let response_text = if let Some(message) = execution_message {
                        ...
                        match invocation.name.as_str() {
                            ...
                            _ => "Recorded by Forge Harness".into(),
                        }
                    };
                    return Ok(json!({
                ...
                Ok(json!({
                    "contentItems": [{ "type": "inputText", "text": "Recorded by Forge Harness" }],
                    "success": true
                }))
            }
            _ => Ok(Value::Null),
