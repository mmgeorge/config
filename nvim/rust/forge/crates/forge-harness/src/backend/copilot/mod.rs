...
pub struct CopilotBackend {
    ...
    closed: AtomicBool,
    shutdown: Mutex<ProviderShutdown>,
...
impl CopilotBackend {
    ...
    pub fn new_with_permission_coordinator(
        ...
        Ok(Self {
            ...
            shutdown: Mutex::new(ProviderShutdown::Idle),
        ...
        let started = Client::start(options)
            ...
            .map_err(|error| {
                if matches!(error.kind(), github_copilot_sdk::ErrorKind::BinaryNotFound { .. }) {
                    anyhow::anyhow!(error).context("Copilot CLI is unavailable. Restore the SDK's pinned CLI cache by rebuilding Forge, or set harness.backends.copilot.command to an installed CLI executable. Explicit configured commands retain precedence")
                } else {
                    anyhow::anyhow!(error).context("start GitHub Copilot SDK client")
                }
            })?;
...
impl Backend for CopilotBackend {
    async fn shutdown(&self) -> Result<()> {
        ...
        if matches!(*shutdown, ProviderShutdown::Idle) {
            ...
            *shutdown = ProviderShutdown::Running(tokio::spawn(async move {
                match client {
                    Some(client) => client.stop().await.map_err(|error| error.to_string()),
                    None => Ok(()),
                }
            }));
        }
        if let ProviderShutdown::Running(task) = &mut *shutdown {
            let result = task.await.unwrap_or_else(|error| Err(error.to_string()));
            *shutdown = ProviderShutdown::Complete(result);
        }
        if let ProviderShutdown::Complete(Err(error)) = &*shutdown {
            anyhow::bail!("Copilot runtime collection failed: {error}");
        }
        self.client.lock().await.take();
        self.session_slot_by_harness.lock().await.clear();
        self.turn_event_by_harness.lock().await.clear();
        self.control_router_by_harness.lock().await.clear();
        self.permission_context_by_harness.lock().await.clear();
        self.completed_event_by_harness.lock().await.clear();
        Ok(())
    }
...
struct ControlToolRouter {
    sender: StandardMutex<Option<mpsc::Sender<ControlToolInvocation>>>,
...
impl ControlToolRouter {
    fn activate(self: &Arc<Self>, context: ControlTurnContext) -> Result<ControlToolStream> {
        ...
        let (active_sender, receiver) = mpsc::channel(32);
    ...
    async fn route(&self, invocation: ControlToolInvocation) -> Result<String> {
        let (active_sender, admission, mut runtime) = {
            ...
            let admission = sender
                .try_reserve_owned()
                .context("Copilot control-tool admission is full or closed")?;
            forge_protocol::outbound::encode(&invocation, 256 * 1024)
                .context("Copilot control-tool invocation exceeds 256 KiB")?;
            let runtime = self
                .runtime
                .lock()
                .map_err(|_| anyhow::anyhow!("Copilot control-tool runtime lock poisoned"))?
        ...
        anyhow::ensure!(
            sender_guard
                .as_ref()
                .is_some_and(|current| current.same_channel(&active_sender)),
            "Copilot control-tool result belongs to a collected turn"
        );
        *self
            .runtime
            .lock()
...
struct ControlToolStream {
    ...
    receiver: mpsc::Receiver<ControlToolInvocation>,
        ...
        let event = BackendEvent {
            ...
            activity: Some(ToolActivity {
                ...
                output: Some(
                    error
                        .clone()
                        .unwrap_or_else(|| "Recorded by Forge Harness".into()),
                ),
                status: Some(
                    if error.is_some() {
                        "failed"
...
mod test {
    ...
    async fn default_cli_resolution_starts_and_collects_without_a_session() {
        ...
        backend
            .client(workspace.path().to_str().unwrap())
            .await
            .unwrap();
        assert_eq!(backend.client_start_count(), 1);
        backend.shutdown().await.unwrap();
        assert!(backend.client.lock().await.is_none());
        assert!(backend.session_slot_by_harness.lock().await.is_empty());
    }

    #[tokio::test]
    async fn shutdown_waiter_cancellation_retains_the_same_collection_task() {
        use super::{Backend, CopilotBackend, ProviderShutdown};
        let backend = CopilotBackend::new(Vec::new()).unwrap();
        let (release, released) = tokio::sync::oneshot::channel();
        *backend.shutdown.lock().await = ProviderShutdown::Running(tokio::spawn(async move {
            released.await.map_err(|error| error.to_string())
        }));
        assert!(
            tokio::time::timeout(std::time::Duration::ZERO, backend.shutdown())
                .await
                .is_err()
        );
        assert!(matches!(
            *backend.shutdown.lock().await,
            ProviderShutdown::Running(_)
        ));
        release.send(()).unwrap();
        backend.shutdown().await.unwrap();
        backend.shutdown().await.unwrap();
        assert_eq!(backend.client_start_count(), 0);
        assert!(backend.client(".").await.is_err());
    }
    ...
    #[tokio::test]
    async fn control_admission_rejects_before_terminal_mutation() {
        let router = Arc::new(ControlToolRouter::default());
        let mut context = ControlTurnContext::inactive(PromptMode::Chat);
        context.has_active_goal = true;
        let mut stream = router.activate(context).unwrap();
        for _ in 0..32 {
            router
                .route(ControlToolInvocation {
                    name: "harness_goal_status".into(),
                    arguments: serde_json::json!({ "status": "working" }),
                })
                .await
                .unwrap();
        }
        let terminal = ControlToolInvocation {
            name: "harness_goal_complete".into(),
            arguments: serde_json::json!({ "summary": "finished" }),
        };
        assert!(
            router
        ...
        assert_eq!(stream.receive().await.unwrap().name, "harness_goal_status");
        assert_eq!(
            router.route(terminal).await.unwrap(),
            "Goal control accepted"
        );
        for _ in 0..31 {
            assert_eq!(stream.receive().await.unwrap().name, "harness_goal_status");
        }
    ...
    #[tokio::test]
    async fn oversized_control_invocation_preserves_runtime_and_capacity() {
        let router = Arc::new(ControlToolRouter::default());
        let mut context = ControlTurnContext::inactive(PromptMode::Chat);
        context.has_active_goal = true;
        let mut stream = router.activate(context).unwrap();
        assert!(
            router
                .route(ControlToolInvocation {
                    name: "harness_goal_complete".into(),
                    arguments: Value::String("x".repeat(256 * 1024)),
                })
                .await
                .unwrap_err()
                .to_string()
