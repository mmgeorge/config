...
use crate::limits::serialized_size;
use anyhow::Result;
...
struct RenderRows<Row> {
    row: Vec<Row>,
    bytes: usize,
    exhausted: bool,
}

impl<Row> Default for RenderRows<Row> {
    fn default() -> Self {
        Self {
            row: Vec::new(),
            bytes: 0,
...
impl<Row: Serialize> RenderRows<Row> {
    fn push(&mut self, row: Row) {
        if self.exhausted {
            return;
        }
        let remaining = (32 * 1024 * 1024usize).saturating_sub(self.bytes);
        let size = serialized_size(&row, remaining);
        if self.row.len() >= 65536 || size.is_err() {
            self.exhausted = true;
            return;
        }
        self.bytes += size.expect("bounded render row");
        self.row.push(row);
    }
    fn extend(&mut self, rows: impl IntoIterator<Item = Row>) {
        for row in rows {
            self.push(row);
            if self.exhausted {
                break;
...
impl PlanRenderer {
    ...
    fn push(
        ...
        let row_count = value.bytes().filter(|byte| *byte == b'\n').count() + 1;
        if row_count > 65536usize.saturating_sub(self.line.len())
            || value.len() + 1 > (8 * 1024 * 1024usize).saturating_sub(self.bytes)
        {
            self.exhausted = true;
            return;
        }
        self.bytes += value.len() + 1;
        let mut anchor = PlanNavigationAnchor {
            line: 0,
            target,
        ...
        for row in value.split('\n') {
            if !row.is_empty() {
                anchor.line = self.line.len() as u32 + 1;
                let Ok(size) = serialized_size(
                    &anchor,
                    (32 * 1024 * 1024usize).saturating_sub(self.metadata_bytes),
                ) else {
                    self.exhausted = true;
                    return;
                };
                self.metadata_bytes += size;
                self.navigation.anchor.push(anchor.clone());
            }
            self.line.push(row.to_owned());
        }
    }
    ...
    fn blank(&mut self) {
        ...
        if self.line.len() >= 65536 || self.bytes >= 8 * 1024 * 1024 {
            self.exhausted = true;
            return;
        }
        self.bytes += 1;
    ...
    fn push_wrapped(
        ...
        let [first_prefix, continuation_prefix] = prefix;
    ...
    fn finish(self) -> Result<RenderedPlan> {
        anyhow::ensure!(
            !self.exhausted,
            "plan rendering exceeds 8 MiB text, 65536 rows, 32 MiB navigation/diagram metadata, or 128 diagram levels"
        );
        debug_assert_eq!(
...
fn wrap_prefixed<'source>(
    content: &'source str,
    first_prefix: &'source str,
    continuation_prefix: &'source str,
    width: usize,
) -> impl Iterator<Item = String> + 'source {
    let mut words = content.split_whitespace().peekable();
    let mut first = true;
    std::iter::from_fn(move || {
        if !first && words.peek().is_none() {
            return None;
        }
        ...
        .to_owned();
        first = false;
        let mut columns = line.chars().count();
        let mut has_content = false;
        while let Some(word) = words.peek() {
            let word_columns = word.chars().count();
            let separator = usize::from(has_content);
            if has_content && columns + separator + word_columns > width {
                break;
            }
            if has_content {
                line.push(' ');
            }
            line.push_str(words.next().expect("peeked plan word"));
            columns += separator + word_columns;
            has_content = true;
        }
        Some(line)
    })
...
fn render_flow_rows(
    ...
    let mut row_list = RenderRows::default();
    ...
    if row_list.exhausted {
        renderer.exhausted = true;
        return;
    }
    let row_list = row_list.row;
...
fn collect_flow_edge_rows(
    row_list: &mut RenderRows<FlowRenderRow>,
    graph: &PlanGraph<'_>,
    edge: &PlanFlowEdge,
    edge_path: &str,
    ...
    if ancestor_last_list.len() > 128 {
        row_list.exhausted = true;
        return;
    }
...
fn render_object_model(
    ...
    if !has_contract {
        ...
        let cells = diagram_entity_group(
            document,
            graph,
    ...
    if contract_cell_list.exhausted || concrete_cell_list.exhausted {
        renderer.exhausted = true;
        return;
    }
    let contract_cell_list = contract_cell_list.row;
    let concrete_cell_list = concrete_cell_list.row;
    renderer.section(
        format!(
            "{}Concrete",
...
fn diagram_entity_group(
    ...
    let mut cell_list = RenderRows::default();
    for (entity_index, _) in entity_list {
        append_entity_cells(
            &mut cell_list,
            document,
            graph,
...
mod test {
    #[test]
    fn renderer_rejects_row_and_text_limits_without_publishing_partial_markdown() {
        let document = crate::plan::document::test_fixture("plan", "overview");
        let mut renderer = PlanRenderer::new(&document);
        for _ in 0..65537 {
            renderer.blank();
        }
        assert_eq!(renderer.line.len(), 65536);
        assert!(renderer.finish().is_err());
        let mut renderer = PlanRenderer::new(&document);
        renderer.section("x".repeat(8 * 1024 * 1024), PlanSection::Title, "/title");
        assert!(renderer.line.is_empty());
        assert!(renderer.finish().is_err());
        assert!(serialized_size(&"escaped\\text", 4).is_err());
        let mut renderer = PlanRenderer::new(&document);
        renderer.section("\n".repeat(65536), PlanSection::Overview, "/overview");
        assert!(renderer.line.is_empty());
        assert!(renderer.finish().is_err());
    }

    #[test]
    fn wrapping_streams_rows_and_diagram_admission_stops_before_retaining_overflow() {
        assert_eq!(
            wrap_prefixed("one two three", "- ", "  ", 9).collect::<Vec<_>>(),
            ["- one two", "  three"]
        );
        assert_eq!(
            wrap_prefixed("", "prefix", "", 9).collect::<Vec<_>>(),
            ["prefix"]
        );
        let mut cells = RenderRows::default();
        cells.bytes = 32 * 1024 * 1024;
        cells.push(DiagramCell {
            text: "new".into(),
            anchor: object_model_anchor(),
        });
        assert!(cells.exhausted && cells.row.is_empty());
    }
    use super::*;
    ...
    fn renders_plan_file_changes_as_a_status_aligned_tree() {
        ...
        let markdown = renderer.finish().unwrap().markdown;
    ...
    fn anchors_every_nonblank_line_to_a_json_path() {
        ...
        assert!(rendered.markdown.contains(overview));
        let source_row: Vec<_> = rendered.markdown.split('\n').collect();
        let anchored_row: Vec<_> = rendered
            .navigation
            .anchor
            .iter()
            .map(|anchor| anchor.line as usize)
            .collect();
        let nonblank_row: Vec<_> = source_row
            .iter()
            .enumerate()
            .filter_map(|(index, row)| (!row.is_empty()).then_some(index + 1))
            .collect();
    ...
    fn orders_each_subtask_without_requiring_the_dependency_parent() {
        ...
        let mut renderer = PlanRenderer::new(&document);
        render_tasks(&mut renderer, &document, &graph);
        let task_markdown = renderer.finish().unwrap().markdown;
