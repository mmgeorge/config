...
- A variant payload field may include the redundant `"kind": "field"` discriminator, but it may also omit it. Harness preserves every supplied member, variant, and payload-field description for the PlanReview info popup, not the Markdown projection.
...
- Give every flow a natural title such as Capture, Sync, or Recovery and a substantive description. State the entry point and observable outcome, then explain the ownership boundary, architectural risk, or independent review reason that makes this flow distinct.
- Every flow has one unrendered `source` and an ordered `edges` array. Harness renders only typed runtime edges, never an action or actor row. An edge `expansion` executes inside that relationship, and edge `branches` represent labeled alternative continuations. Always send `edges`, `branches`, and `expansion` as complete arrays, including empty arrays.
- Put work performed by a callee inside that edge's `expansion`. Put every branch condition in `condition` and its nonempty continuation in `edges`. Never infer nesting from adjacency.
- An expansion must explain material work through a `construct`, `call`, `read`, `write`, `send`, or `emit` edge, or through a meaningful branch. Never add an expansion that only returns a value already carried by its parent edge.
...
- Choose the exact edge relation: `construct` creates the target, `call` invokes a callable, `read` obtains data through a callable, `write` mutates or persists through a callable, `send` transfers a typed payload, `emit` produces a typed payload, and `return` transfers a typed payload back to the target.
- Put `relation` directly on the edge as a snake-case string. Give every `call`, `read`, and `write` edge one sibling `callable` with `kind: "function"` or `"method"` and its bare identifier in `name`. Never put parentheses, arguments, receiver names, or prose in the callable name.
- Callable edges may carry `return_type: {"value_type":"TypeName","error_type":"ErrorType"}`. Omit `error_type` for an infallible return. Harness renders the value type inline, while an explicit branch handles an error type when that behavior matters. `send`, `emit`, and `return` require a sibling `payload_type`.
...
- Put every transferred type in `payload_type` and every callable output in structured `return_type`. Inputs such as paths and files belong in the typed payload or callable context, never as a callable receiver.
...
- Edge order controls reviewer presentation. Runtime meaning comes from explicit relations, expansions, and branches, never prose action labels.
...
- Include every retained nested member, variant, field, edge, branch, task file, and subtask in each set resource.
...
mod test {
    ...
    fn planning_contract_requires_structured_submission_without_native_mode() {
        ...
        assert!(prompt.contains("Harness renders only typed runtime edges"));
        assert!(prompt.contains("Every flow has one unrendered `source`"));
        assert!(prompt.contains("edge `branches` represent labeled alternative continuations"));
        assert!(prompt.contains("Never infer nesting from adjacency"));
        assert!(prompt.contains("An expansion must explain material work"));
        assert!(prompt.contains("Never add an expansion that only returns a value"));
        assert!(prompt.contains("sibling `callable`"));
        ...
        assert!(prompt.contains("State the entry point and observable outcome"));
        assert!(
            prompt.contains("ownership boundary, architectural risk, or independent review reason")
        );
        assert!(
            prompt.contains(r#"return_type: {"value_type":"TypeName","error_type":"ErrorType"}"#)
        );
        assert!(prompt.contains("`payload_type`"));
        assert!(prompt.contains("show concrete construction and invocation edges"));
        assert!(
            prompt.contains(
                "Runtime meaning comes from explicit relations, expansions, and branches"
            )
        );
        assert!(prompt.contains("`construct` creates the target"));
        assert!(prompt.contains("Put `relation` directly on the edge as a snake-case string"));
        assert!(prompt.contains("`send`, `emit`, and `return` require a sibling `payload_type`"));
        ...
        assert!(prompt.contains("still describe its concrete work through nested edges"));
        assert!(prompt.contains(r#""source": {"#));
        assert!(prompt.contains(r#""relation": "call""#));
