...
pub fn apply_plan_mutation(document: &mut PlanDocument, mutation: PlanMutation) -> Result<()> {
        ...
        || delete.entity_changes.is_some()
    {
        apply_resource_patch(
            ...
            ResourceSchema {
                resource: "program entity",
                collection: "entity_changes",
                semantic_key_field: "name",
                semantic_key: |entity| entity.name.as_str(),
                rename_value: |entity, name| entity.name = name,
                prepare_value: prepare_entity,
            },
        )?;
    }
    if rename.dependencies.is_some() || set.dependencies.is_some() || delete.dependencies.is_some()
    {
        apply_resource_patch(
            ...
            ResourceSchema {
                ...
                semantic_key_field: "name",
                semantic_key: |dependency| dependency.name.as_str(),
                rename_value: |dependency, name| dependency.name = name,
                prepare_value: prepare_dependency,
            },
        )?;
    }
    if rename.flows.is_some() || set.flows.is_some() || delete.flows.is_some() {
        apply_resource_patch(
            ...
            ResourceSchema {
                ...
                semantic_key: |flow| flow.title.as_str(),
                rename_value: |flow, title| flow.title = title,
                prepare_value: prepare_flow,
            },
        )?;
    }
...
struct ResourceSchema<T> {
    resource: &'static str,
    collection: &'static str,
    semantic_key_field: &'static str,
    semantic_key: fn(&T) -> &str,
    rename_value: fn(&mut T, String),
    prepare_value: fn(Option<&T>, &mut T),
}
