...
static SYSTEM_PREFIX: OnceLock<Option<PathBuf>> = OnceLock::new();
...
pub fn exe_invocation() -> &'static Path {
    if cfg!(windows) {
        ...
        EXECUTABLE_PATH
            .as_deref()
            .unwrap_or(Path::new(git::EXE_NAME))
    } else {
        Path::new("git")
    }
}
...
pub fn installation_program(name: &str) -> Option<PathBuf> {
    ...
    core_dir_program(name).or_else(|| {
        if cfg!(windows) {
            auxiliary::find_git_associated_windows_executable(name)
                .map(|executable| executable.program.into())
        } else {
            None
        }
    })
    ...
    F: Fn() -> Option<&'static Path>,
{
    ...
    let one_past_prefix = path.components().enumerate().find_map(|(idx, c)| {
        matches!(c,std::path::Component::Normal(name) if name.to_str() == Some("libexec"))
            .then_some(idx)
    })?;
    Some(
        path.components()
            .take(one_past_prefix.checked_sub(1)?)
            .collect(),
    )
}
...
/// Installs an absolute system prefix before its first lookup in this process.
///
/// Returns the supplied path if it is relative or a lookup already initialized the prefix.
/// Callers must validate persisted locations and honor discovery overrides before calling this.
/// Configuration and attributes continue to load from their normal paths under this prefix.
pub fn set_system_prefix(prefix: PathBuf) -> Result<(), PathBuf> {
    if !prefix.is_absolute() {
        return Err(prefix);
    }
    SYSTEM_PREFIX
        .set(Some(prefix))
        .map_err(|prefix| prefix.expect("supplied prefix"))
}
